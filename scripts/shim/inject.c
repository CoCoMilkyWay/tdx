...
// Wine 9.0 支持这套跨进程 API（NtAllocateVirtualMemory/NtWriteVirtualMemory/NtCreateThreadEx）。
// 32 位匹配 TdxW。编译见 build.sh。
// 用法：wine inject.exe <shim.dll 的 Windows 路径，如 Z:\home\chuyin\work\tdx\scripts\shim\shim.dll>
...
int main(int argc, char **argv) {
    ...
    DWORD pid = find_proc("tdxw.exe");
    if (!pid) { printf("找不到 tdxw.exe\n"); return 1; }
    printf("[*] tdxw.exe pid=%lu\n", pid);

    HMODULE k32 = GetModuleHandleA("kernel32.dll");
    LPVOID load = (LPVOID)GetProcAddress(k32, "LoadLibraryA");
    if (!load) { printf("拿不到 LoadLibraryA\n"); return 2; }

    HANDLE hp = OpenProcess(PROCESS_ALL_ACCESS, FALSE, pid);
    if (!hp) { printf("OpenProcess 失败 err=%lu\n", GetLastError()); return 3; }

    SIZE_T len = strlen(dll) + 1;
    LPVOID remote = VirtualAllocEx(hp, NULL, len, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    if (!remote) { printf("VirtualAllocEx 失败 err=%lu\n", GetLastError()); return 4; }
